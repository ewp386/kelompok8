            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('invoice.update', $invoice->id) }}">
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="id_transaksi_item" class="block font-medium text-sm text-gray-700">ID Transaksi Item</label>
                            <input type="text" name="id_transaksi_item" id="id_transaksi_item" type="text" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('id_transaksi_item', $invoice->id_transaksi_item) }}" />
                            @error('id_transaksi_item')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="text" name="id_transaksi" id="id_transaksi" type="text" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('id_transaksi', $invoice->id_transaksi) }}" />
                            @error('id_transaksi')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
